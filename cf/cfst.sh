...
# 如果指定了-f参数，直接对指定文件测速并退出
if [ -n "$input_file" ]; then
  if [ ! -f "$input_file" ]; then
    echo "指定的文件不存在: $input_file"
    exit 1
  fi

  # 判断文件内容为IPv4还是IPv6
  first_ip=$(head -n 1 "$input_file")
  if [[ "$first_ip" == *:* ]]; then
    ip_version=6
    outfile="custom-6.csv"
  else
    ip_version=4
    outfile="custom-4.csv"
  fi

  ./cf -ips $ip_version -infile "$input_file" -outfile "$outfile"
  result
  echo "测试完成，结果文件: $outfile"
  exit 0
fi
